...
export default function SearchScreen() {
  ...
  const loadHistory = async () => {
    const rows = await db.getAllAsync(
      "SELECT term FROM search_history ORDER BY id DESC LIMIT 10;"
    );
  ...
  const saveSearch = async (term) => {
    ...
    try {
      await db.runAsync(
        "INSERT OR IGNORE INTO search_history (term) VALUES (?);",
        [term.trim()]
      );
      loadHistory();
    } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    if (q)
      arr = arr.filter((d) => d.name.toLowerCase().includes(q.toLowerCase()));
